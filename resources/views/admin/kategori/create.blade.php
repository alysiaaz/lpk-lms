...
<div class="bg-white p-8 rounded-2xl border border-admin-border shadow-sm max-w-lg">
    ...
    <form action="{{ route('admin.kategori.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="nama" value="{{ old('nama') }}" class="w-full border border-admin-border p-2.5 rounded-lg focus:ring-2 focus:ring-admin-accent focus:border-admin-accent outline-none" placeholder="Misal: Desain Grafis">
